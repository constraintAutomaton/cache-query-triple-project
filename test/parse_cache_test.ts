...
describe("parseCache", () => {
    ...
    it("should return an empty cache give a resource having partial caches definition", async () => {
        ...
        <foo1> a "${Vocabulary.QUERY_CLASS.value}" ;
            <${Vocabulary.QUERY_IRI_PREDICATE.value}> <bar> .
        <foo2> <${Vocabulary.QUERY_IRI_PREDICATE.value}> <bar> ;
    ...
    it("should return an empty cache given a resource having a full cache definition but no endpoint", async () => {
        const string_triples = `
        <foo>  <${Vocabulary.QUERY_IRI_PREDICATE.value}> <bar> ;
            <${Vocabulary.RESULT_IRI_PREDICATE.value}> <fooR> ;
    ...
    it("should return a cache entry given a resource having a full cache definition", async () => {
        ...
        const entryEndpoint = cache.get("endpoint1");
        ...
        expect(new Set(entryQuery?.endpoints)).toStrictEqual(new Set(["endpoint2", "endpoint3"]));
    ...
    it("should return multiple cache entries given a resource having a full cache definitions", async () => {
        ...
        const entryEndpoint1 = cache.get("endpoint1");
        ...
        expect(new Set(entryQueryBar?.endpoints)).toStrictEqual(new Set(["endpoint2", "endpoint3"]));
        expect(entryQueryBar?.resultUrl).toBe("fooR");

        const entryQueryBar1 = entryEndpoint1?.get("bar1");
        expect(entryQueryBar1).toBeDefined();

        expect(entryQueryBar1?.endpoints).toStrictEqual([]);
        expect(entryQueryBar1?.resultUrl).toBe("fooR1");

        const entryEndpoint4 = cache.get("endpoint4");
        ...
        expect(new Set(entryQueryFoo?.endpoints)).toStrictEqual(new Set(["endpoint5"]));
